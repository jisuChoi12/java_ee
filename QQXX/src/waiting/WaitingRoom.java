...
public class WaitingRoom extends JFrame implements ActionListener, Runnable, ListSelectionListener {
	...
	private ArrayList<String> nicksss; // 20190811
	...
	public void enterGame(int index, String roomName) {
		...
		nicksss = roomdao.getNicksss(roomName);
...
//			}
			...
			// 20190811
			if ((!nicksss.get(0).equals("0")) && (!nicksss.get(1).equals("0"))) {
				JOptionPane.showMessageDialog(this, "방이 다 참");
			} else {
				int ans = JOptionPane.showConfirmDialog(this, "입장 하시겠습니까?", "입장", JOptionPane.YES_NO_OPTION,
						JOptionPane.PLAIN_MESSAGE);
				if (ans == 0) { // 입장 버튼 눌렀을때
					System.out.println("<전> player1: " + nicksss.get(0) + "   " + "player2: " + nicksss.get(1));
					if (nicksss.get(0).equals("0") && nicksss.get(1).equals("0")) {// 빈 방
						// 디비에 player update
						int su = roomdao.updateRoomP1(dto.getNickName(), roomName, 1); // 방 인원 = 1명
						new GameWindow(dto.getNickName(), roomName).service();
						nicksss = roomdao.getNicksss(roomName);
						System.out.println("<후> player1: " + nicksss.get(0) + "   " + "player2: " + nicksss.get(1));
					} else if ((!nicksss.get(0).equals("0")) && nicksss.get(1).equals("0")) { // 이미 한명 들어가 있음
						int su = roomdao.updateRoomP2(dto.getNickName(), roomName, 2);
						new GameWindow(dto.getNickName(), roomName).service();
						nicksss = roomdao.getNicksss(roomName);
						System.out.println("<후> player1: " + nicksss.get(0) + "   " + "player2: " + nicksss.get(1));
					}
...
//				new GameWindow(dto.getNickName(), roomName).service(); // 닉네임, 방이름 넘겨주기 20190811
