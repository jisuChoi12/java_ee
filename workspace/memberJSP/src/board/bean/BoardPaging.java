...
public class BoardPaging {
	...
	public void makePagingHTML(){
		...
		if(startPage>pageBlock) { // [이전]이 나오는 시점
			pagingHTML.append("[<a id='prev' href='boardList.jsp?pg="+(startPage-1)+"'>이전</a>]"); // [이전]을 누르면 startPage-1 페이지로 이동 
		}
		...
		if(endPage<totalPage) { // [다음]이 나오는 시점
			pagingHTML.append("[<a id='next' href='boardList.jsp?pg="+(endPage+1)+"'>다음</a>]"); // [다음]을 누르면 endPage+1 페이지로 이동
		}
